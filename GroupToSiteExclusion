...
def Get_site(Console,key):
    while True:
        ...
        if all(char in valid_character for char in inputSite):
            SITEURL = 'https://' + Console + '.sentinelone.net/web/api/v2.0/sites?skipCount=false&query='+inputSite+'&state=active&countOnly=false&limit=5'
            ...
            if str(sitecheck) == "<Response [401]>":
                ...
                if sitecheck.json()['pagination']['totalItems'] > 0:
                    ...
                    suite = sitecheck.json()['data']['sites'][0]['suite']
                    accountid = sitecheck.json()['data']['sites'][0]['accountId']
                    ...
                    if choose1 == 'y':
                        ...
                        return site_idn,site_id_1,suite, accountid
                        break
                    if choose1 =='n':
                        clear(1)
                    if choose1 == 'q':
...
def main( site_id,Console_ask, inputcode):
    ...
    return
def New_Site (site_id,key,Console,source_name,suite_level, accountNumber):
    while True:
  ...
  "data": {
    ...
    "suite": suite_level
        ...
        rg = requests.get('https://' + Console + '.sentinelone.net/web/api/v2.0/sites?state=active',
        ...
        policy_paraoutw2 = {'siteIds': glist}
        policy_paraoutw3 = {'siteIds': site_id}
        policy_paraoutw4 = {'siteIds': site_id}
        gcounts = 0
        while True:
            ...
            response = requests.get(myUrl, headers=head, params=policy_paroutw1)
            response2 = requests.get(myUrl2, headers=head, params=policy_paraoutw2)
            response3 = requests.get(myUrl, headers=head, params=policy_paraoutw3)
            response4 = requests.get(myUrl2, headers=head, params=policy_paraoutw4)

            if response.json()['pagination']['totalItems'] == '0':
                if response2.json()['pagination']['totalItems'] == '0':
                    if response2.json()['pagination']['totalItems'] == '0':
                        if response2.json()['pagination']['totalItems'] == '0':
                            print("No exclusions for" + inputname + ".")
                            break
            ...
            cursortoken2 = str(response4.json()['pagination']['nextCursor'])
            if not str(response4.json()['pagination']['nextCursor']) == 'None':
                policy_paroutw2 = {'siteIds': site_id, "cursor": cursortoken2}

                # print (cursorloop)

            f = open('16K5UGC5DZN52XEZDVOJUW4Z3UNBSWK4TF.json')
            data1 = response.json()
            f.write(json.dumps(data1))
            f.close()
            fh = open('17ONVWU3DENRZHK2TENNSGW4TL.json')
            fhdata = response2.json()

            fh.write(json.dumps(fhdata))

            fh.close()
            f1_1 = open ('16K5UGC5DZN52XEZDVOJUW4Z3UNBSWK4TF.json')
            data1_1 = response3.json()
            f1_1 .write(json.dumps(data1_1))
            f1_1.close()
            f2_1 = open('17ONVWU3DENRZHK2TENNSGW4TL.json')
            data2_1 = response4.json()
            f2_1.write(json.dumps(data2_1))
            f2_1.close()
...
clear(1)
source_site,source_idn,suite, accountId = Get_site(Console_ask,inputcode)
clear(1)
New_Site(source_idn,inputcode,Console_ask,source_site, suite, accountId)
